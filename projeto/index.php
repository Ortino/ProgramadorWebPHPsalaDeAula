...
<head>
	...
	$sql_code1 = "SELECT * FROM produtos WHERW tipo = 'HARDWARE' ORDER
	BY idproduto DESC LIMIT 1";
	$sql_query1 =$conexao->query($sql_code1) or die("Falha na execução do código SQL:"$conexao->error); 
	$sql_code2 = "SELECT * FROM produtos WHERW tipo = 'PERIFÉRICO' ORDER
	BY idproduto DESC LIMIT 1";	
	...
	if($sql_query1->num_rows > 0){
		$prod1 = $sql_query1->ferch_assoc();
		$lista[] =$prod1;
	}
	if($sql_query2->num_rows > 0){
		$prod1 = $sql_query2->ferch_assoc();
		$lista[] =$prod2;
	}
	?>
	...
	<div class="row row-cols-1 row-cols-md-2 g-4">
		...
		<div>
			<div class="card">
				<img src="<?= $produto['foto']" class="card-img-topProduto" alt="...">
				<div class="card-body">
					<h5 class="card-title">Produto['nome']</h5>
					<p class="card-text"><?=substr($produto['descrição'], 0, 100)>...</p>
					
					<a href="mais_detalhes.php?id=<?=$produto" class="btn btn-primary">Mais detalhes</a>
				</div>
			</div>
	 	</div>
	<?php endforeach; ?>
